...
const MinhasRemessas = () => {
  ...
  // Buscar apenas as remessas do usuário logado
  const buscarMinhasRemessas = async () => {
    if (!user?.id) return;
    setLoading(true);
    try {
      ...
      console.error('Erro ao buscar minhas remessas:', err);
  ...
  const exportarExcel = () => {
    ...
    try {
      ...
      saveAs(
        blob,
        `minha-remessa-${remessaSelecionada?.nr_remessa}-${hoje}.xlsx`,
      );
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col items-stretch justify-start py-3 px-2">
      <PageTitle
        title="Minhas Remessas"
        subtitle="Acompanhe o status das suas solicitações de remessa"
        icon={Package}
            ...
            <div className="flex justify-center items-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-500 text-sm mb-1">
                  Você ainda não criou nenhuma remessa
                </p>
                <p className="text-gray-400 text-xs">
                  Crie remessas na página de Licitação de Títulos.
                </p>
              </div>
            </div>
          ) : (
