...
const DashboardRevenda = () => {
  ...
  const buscarDados = async () => {
    ...
    try {
      const responseRevenda = await apiClient.sales.faturamentoRevenda({
        dataInicio,
        dataFim,
      });
      ...
      const normalizedRevenda = normalizeResponseToArray(responseRevenda);
      setDadosRevenda(normalizedRevenda);
      ...
      const responseMesAnterior = await apiClient.sales.faturamentoRevenda({
      ...
      setError('Erro ao carregar dados de faturamento de revenda');
    } finally {
      setLoading(false);
    }
  };
  ...
  const dadosConsolidados = useMemo(() => {
    if (!dadosRevenda.length) return null;

    const consolidado = dadosRevenda.reduce(
      (acc, item) => {
  ...
  const dadosConsolidadosMesAnterior = useMemo(() => {
    ...
    const consolidado = dadosMesAnterior.reduce(
      ...
      { valor_liquido_saida: 0, valor_liquido_entrada: 0 },
    );
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      <PageTitle
        title="Dashboard Revenda"
        subtitle="Análise completa do faturamento do canal Revenda"
        icon={UsersFour}
      ...
      {loading ? (
        ...
        <>
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl rounded-xl bg-white mb-6">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                <UsersFour size={20} className="text-[#000638]" />
                <CardTitle className="text-lg font-bold text-[#000638]">
                  Detalhamento Revenda
                </CardTitle>
              </div>
              <CardDescription>
                Resumo financeiro completo do canal Revenda
              </CardDescription>
            </CardHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            {dadosPizzaEmpresa && (
              <Card
                ...
                onClick={() =>
                  setModalGrafico({
                    title: 'Revenda - Faturamento por Empresa',
              ...
              >
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-lg font-bold text-[#000638]">
                      Revenda - Faturamento por Empresa
                    </CardTitle>
                  </div>
                  <CardDescription>
                    Distribuição do faturamento por empresa (Revenda)
                  </CardDescription>
                </CardHeader>
                <CardContent>
...
export default DashboardRevenda;
