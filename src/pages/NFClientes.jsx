...
const NFClientes = () => {
  ...
  const FinancialURL =
    'https://apigestaocrosby-bw2v.onrender.com/api/financial/';
  ...
  const buscarClientes = async () => {
    ...
    try {
      ...
      const response = await fetch(`${FinancialURL}buscar-clientes?${query}`);
  ...
  const buscarDados = async () => {
    ...
    try {
      console.log(
        `🔍 Buscando notas fiscais para cd_pessoa: ${cdCliente}, período: ${periodo.dt_inicio} a ${periodo.dt_fim}`,
      );

      const res = await fetch(
        `${BaseURL}meuspedidos?cd_pessoa=${cdCliente}&dt_inicio=${periodo.dt_inicio}&dt_fim=${periodo.dt_fim}`,
      );

      if (!res.ok) {
        console.warn(`Erro ao buscar cliente ${cdCliente}: HTTP ${res.status}`);
        setDados([]);
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'NF Clientes');
      ...
      const nomeArquivo = `nf-clientes-${hoje}.xlsx`;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col items-stretch justify-start py-3 px-2">
      <PageTitle
        title="NF Clientes"
        subtitle="Consulte Notas Fiscais por Cliente"
        icon={Article}
        ...
        >
          <div className="mb-2">
            ...
            <span className="text-xs text-gray-500 mt-1">
              Busque o cliente pelo nome ou fantasia e selecione o período
            </span>
          </div>

          {/* Seção de Busca de Cliente */}
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-3">
            <h3 className="text-sm font-semibold text-[#000638] mb-2 flex items-center gap-1">
              <User size={14} />
              Buscar Cliente
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="flex items-center gap-1 bg-[#000638] text-white px-3 py-1.5 rounded-lg hover:bg-[#fe0000] disabled:opacity-50 disabled:cursor-not-allowed transition-colors h-7 text-xs font-bold shadow-md tracking-wide uppercase"
                >
                  {buscandoClientes ? (
                    <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-blue-800">
                      Cliente Selecionado: {clienteSelecionado.nm_pessoa}
                    </p>
                    <p className="text-xs text-blue-600">
                      Código: {clienteSelecionado.cd_pessoa} | Fantasia:{' '}
                      {clienteSelecionado.nm_fantasia || 'N/A'}
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="flex justify-center items-center py-12">
              <div className="text-center">
                <div className="text-gray-500 text-sm mb-2">
                  Clique em "Buscar NFs" para carregar as informações
                </div>
                <div className="text-gray-400 text-xs">
                  Selecione o cliente e o período desejados
                </div>
              </div>
            </div>
          ) : dados.length === 0 ? (
