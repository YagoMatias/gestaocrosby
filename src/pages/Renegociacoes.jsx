...
const FORNECEDORES_RENEGOCIADOS = [
  ...
  { cd: 6499, nome: 'LUNELLI TEXTIL LTDA', diaVencimento: null, inicio: null },
  { cd: 6975, nome: 'VICUNHA', diaVencimento: null, inicio: null },
];
...
const Renegociacoes = () => {
  ...
  const exportarExcel = useCallback(() => {
    const aoa = [['Fornecedor', 'Vencimento', ...MESES]];
    for (const linha of linhas) {
      ...
      aoa.push(['', '', ...linha.meses.map((c) => c.total || 0)]);
    }
    aoa.push([]);
    aoa.push(['TOTAL', '', ...totaisMes]);

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 30 }, { wch: 12 }, ...MESES.map(() => ({ wch: 12 }))];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Renegociações');
    XLSX.writeFile(wb, `renegociacoes-${ano}.xlsx`);
  }, [linhas, totaisMes, ano]);
                ...
                <tbody>
                  {linhas.map((linha) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-right font-bold text-[#000638] border-l border-gray-100">
                        {formatarMoeda(linha.totalAno)}
                        {linha.totalAberto > 0 && (
                          <span className="block text-[9px] text-amber-600 font-semibold">
                            {formatarMoeda(linha.totalAberto)} em aberto
                          </span>
                        )}
                      </td>
                    </tr>
