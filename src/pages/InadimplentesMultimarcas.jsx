...
const InadimplentesMultimarcas = () => {
  ...
  const fetchDados = async () => {
    try {
      ...
      if (filtroCliente) params.nm_cliente = filtroCliente;
  ...
  const dadosFiltrados = useMemo(() => {
    return dados.filter((item) => {
      const matchCliente =
        !filtroCliente ||
        item.nm_cliente?.toLowerCase().includes(filtroCliente.toLowerCase());
      ...
      return matchCliente && estaAtrasado;
    });
  }, [dados, filtroCliente]);
  ...
  const topClientes = useMemo(() => {
    ...
    const sorted = [...clientesAgrupados].sort((a, b) => b.valor_total - a.valor_total).slice(0, 10);
    return sorted.map((c) => {
      const diasMax = (c.faturas || []).reduce((max, f) => {
        if (!f.dt_vencimento) return max;
        const diff = Math.floor((new Date() - new Date(f.dt_vencimento)) / (1000 * 60 * 60 * 24));
  ...
  const graficoPrincipalData = {
    labels: topClientes.map((c) => (c.nm_cliente ? c.nm_cliente : c.cd_cliente)),
    datasets: [
      {
