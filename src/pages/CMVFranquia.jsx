...
const CMVFranquia = () => {
  ...
  const [filtros, setFiltros] = useState({
    ...
    // classificação fixa e oculta: sempre [4]
    nr_transacao: '',
    cd_pessoa: '',
    cd_empresa: '',
  });
  ...
  const buscar = async () => {
    try {
      ...
      const resp = await api.sales.cmvfranquia(params);
      if (!resp.success) {
  ...
  const totais = useMemo(() => {
    let totalLiquido = 0;
    let totalBruto = 0;
    let totalDevolucoes = 0; // soma de devoluções (líquido) como positivo
    let totalCMV = 0; // CMV líquido (soma - devoluções)
    let totalFrete = 0; // soma simples de frete rateado
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      <PageTitle
        title="CMV FRANQUIA"
        subtitle="Consulta da rota /cmvfranquia (view materializada cmvfranquia)"
        icon={ChartLineUp}
