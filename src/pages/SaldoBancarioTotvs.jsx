...
const SaldoBancarioTotvs = () => {
  ...
  const toggleLinhaSelecionada = useCallback((index) => {
    setLinhasSelecionadas((prev) => {
      ...
      nova.has(index) ? nova.delete(index) : nova.add(index);
      return nova;
    });
  }, []);
  ...
  const contasSelecionadas = contas.filter((c) =>
    filtros.nr_ctapes.includes(c.numero),
  );
  ...
  const [ordenacaoSaldo, setOrdenacaoSaldo] = useState({
    campo: 'nome',
    direcao: 'asc',
  });
  ...
  const getSortIconSaldo = useCallback(
    (campo) => {
      if (ordenacaoSaldo.campo !== campo)
        return (
          <CaretUpDown size={12} className="opacity-50 inline-block ml-1" />
        );
      return ordenacaoSaldo.direcao === 'asc' ? (
        <CaretUp size={12} className="inline-block ml-1" />
      ) : (
        <CaretDown size={12} className="inline-block ml-1" />
      );
    },
    [ordenacaoSaldo],
  );
  const saldoOrdenado = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      // Numeros
      if (
        ['creditos', 'debitos', 'saldo', 'saldoExtrato'].includes(
          ordenacaoSaldo.campo,
        )
      ) {
  ...
  const saldoFiltradoGrupo = useMemo(() => {
    ...
    return saldoFiltrado.filter((c) => {
      ...
      if (g === 'IRMAOS CR' || g === 'IRMÃOS CR')
        return nome.includes('IRMAOS CR') || nome.includes('IRMÃOS CR');
  ...
  return (
    <ErrorBoundary
      ...
      onError={(error, errorInfo) => {
        console.error('SaldoBancarioTotvs Error:', error, errorInfo);
      }}
    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-3 gap-y-2 w-full mb-4">
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  readOnly
                  disabled
                  className="border border-[#000638]/30 rounded-lg px-3 py-2 w-full bg-gray-100 text-[#000638] cursor-not-allowed"
                />
              </div>
              <div className="flex flex-col">
                <label className="block text-xs font-semibold mb-1 text-[#000638]">
            ...
            <table className="w-full border-collapse">
              <thead className="bg-[#000638] text-white">
                <tr>
                  <th
                    ...
                    onClick={() => handleSortSaldo('nome')}
                  >
                    <span className="inline-flex items-center">
                      Conta {getSortIconSaldo('nome')}
                    </span>
                  </th>
                  <th
                    className="px-4 py-3 text-right text-sm font-semibold cursor-pointer"
                    onClick={() => handleSortSaldo('creditos')}
                  >
                    <span className="inline-flex items-center">
                      Créditos {getSortIconSaldo('creditos')}
                    </span>
                  </th>
                  <th
                    className="px-4 py-3 text-right text-sm font-semibold cursor-pointer"
                    onClick={() => handleSortSaldo('debitos')}
                  >
                    <span className="inline-flex items-center">
                      Débitos {getSortIconSaldo('debitos')}
                    </span>
                  </th>
                  <th
                    className="px-4 py-3 text-right text-sm font-semibold cursor-pointer"
                    onClick={() => handleSortSaldo('saldo')}
                  >
                    <span className="inline-flex items-center">
                      Saldo {getSortIconSaldo('saldo')}
                    </span>
                  </th>
                  <th
                    className="px-4 py-3 text-right text-sm font-semibold cursor-pointer"
                    onClick={() => handleSortSaldo('saldoExtrato')}
                  >
                    <span className="inline-flex items-center">
                      Saldo Extrato {getSortIconSaldo('saldoExtrato')}
                    </span>
                  </th>
                </tr>
              </thead>
              ...
              <tfoot className="bg-gray-100 border-t-2 border-[#000638]">
                <tr>
                  ...
                  <td
                    className={`px-4 py-3 text-right font-bold ${
                      saldoFiltradoGrupo.reduce((acc, c) => acc + c.saldo, 0) >
                      0
                        ? 'text-green-600'
                        : saldoFiltradoGrupo.reduce(
                            (acc, c) => acc + c.saldo,
                            0,
                          ) < 0
                        ? 'text-red-600'
                  ...
                  >
                    {saldoFiltradoGrupo
                      .reduce((acc, c) => acc + c.saldoExtrato, 0)
                      .toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL',
                      })}
                ...
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-2 text-xs font-semibold text-[#000638]"
                  >
