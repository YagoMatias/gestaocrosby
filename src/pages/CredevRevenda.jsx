...
const CredevRevenda = () => {
  ...
  const fetchDados = async () => {
    ...
    setErro('');
    try {
      console.log('🔍 Buscando dados de CREDEV Revenda...');

      const result = await apiClient.financial.credevRevenda();

      if (result.success) {
        console.log('✅ Dados CREDEV Revenda recebidos:', {
      ...
      console.error('❌ Erro ao buscar dados CREDEV Revenda:', err);
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'CREDEV Revenda');
      ...
      const nomeArquivo = `credev-revenda-${hoje}.xlsx`;
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col items-stretch justify-start py-8 px-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">Saldos de CREDEV - Canal Revenda</p>
        </div>
      </div>
        ...
        <form className="flex flex-col bg-white p-3 rounded-lg shadow-lg w-full max-w-4xl mx-auto border border-[#000638]/10">
          <div className="mb-2">
            ...
            <span className="text-xs text-gray-500 mt-1">
              Configurações para análise CREDEV Revenda
            </span>
          </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-start mb-3">
            <h2 className="text-xl font-bold text-[#000638]">
              Detalhamento CREDEV por Revenda
            </h2>
            {dadosFiltrados.length > 0 && (
              <button
