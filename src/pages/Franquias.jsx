...
const Franquias = () => {
  ...
  const fetchDados = async (empresasParam = empresasSelecionadas) => {
    ...
    try {
      ...
      const res = await fetch(`https://apigestaocrosby-bw2v.onrender.com/api/sales/faturamento-franquia?${params.toString()}`);
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-1 rounded-xl w-64 bg-white cursor-pointer">
            ...
            <CardContent className="pt-0 px-4 pb-4">
              <div className="text-2xl font-extrabold text-red-700 mb-1">
                {loading ? <LoadingCircle size={24} /> : custoTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </div>
              ...
              {loading ? <LoadingCircle size={24} /> : (() => {
                ...
                return (
                  <div className="flex flex-col gap-1 items-start">
                    <span className="text-2xl font-extrabold text-orange-700">{cmv !== null ? (cmv * 100).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + '%' : '-'}</span>
