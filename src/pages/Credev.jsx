...
import { ArrowClockwise, CurrencyDollar, Calendar, Storefront, Users, MagnifyingGlass, X, CaretUp, CaretDown, DownloadSimple, CircleNotch } from '@phosphor-icons/react';
...
const Credev = () => {
      ...
      if (result.success) {
        ...
        const nomesFantasiaUnicos = result.data
          .filter(item => {
            ...
            const temEmpresaValida = !isNaN(codigoEmpresa) && codigoEmpresa < 5999;
            ...
            return item.nm_fantasia && temSaldoPositivo && temDataValida && temEmpresaValida && naoEhPessoa56;
          })
          .map(item => ({
  ...
  const isSaldoPositivo = (saldoOriginal) => {
    if (saldoOriginal === null || saldoOriginal === undefined || saldoOriginal === '') {
      return false;
    }
    ...
    if (typeof saldoOriginal === 'string') {
      ...
      const saldoLimpo = saldoOriginal.replace(/[^\d.,-]/g, '').replace(',', '.');
  ...
  const dadosFiltrados = useMemo(() => {
    return dados.filter(item => {
      ...
      const matchDocumento = !filtroDocumento || 
        (item.tp_documento && item.tp_documento.toLowerCase().includes(filtroDocumento.toLowerCase()));
  ...
  const opcoesDocumento = useMemo(() => {
    const documentos = [...new Set(dados
      .filter(item => {
        // Verificar se é do tipo CREDEV
        if (item.tp_documento !== 'CREDEV') {
          return false;
        ...
        return temSaldoPositivo && temDataValida && temEmpresaValida && naoEhPessoa56;
      })
    ...
    return [...dados].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case 'dt_ultimocredito':
          aValue = a.dt_ultimocredito ? new Date(a.dt_ultimocredito) : new Date(0);
          bValue = b.dt_ultimocredito ? new Date(b.dt_ultimocredito) : new Date(0);
          break;
        default:
    ...
    try {
       ...
       const dadosParaExportar = sortDados(dadosFiltrados).map((item, index) => {
         return {
           ...
           'Última Movimentação': formatarDataBR(item.dt_ultimocredito) === '--' ? '' : formatarDataBR(item.dt_ultimocredito)
         };
       });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-500 cursor-not-allowed outline-none transition-colors"
              disabled
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8 max-w-7xl mx-auto">
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-1 rounded-xl bg-white">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-sm font-bold text-blue-700">Total de Registros</CardTitle>
            </div>
            </CardHeader>
            <CardContent className="pt-0 px-4 pb-4">
              <div className="text-lg font-extrabold text-blue-600 mb-1 break-words">
                {loading ? <CircleNotch size={24} className="animate-spin text-blue-600" /> : dadosFiltrados.length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-1 rounded-xl bg-white">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-sm font-bold text-green-700">Saldo Total</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="pt-0 px-4 pb-4">
              <div className="text-lg font-extrabold text-green-600 mb-1 break-words">
                {loading ? <CircleNotch size={24} className="animate-spin text-green-600" /> : 
                  formatarMoeda(dadosFiltrados.reduce((acc, item) => acc + (Number(item.vl_saldo) || 0), 0))
                }
              </div>
              <CardDescription className="text-xs text-gray-500">Soma de todos os saldos</CardDescription>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-1 rounded-xl bg-white">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-sm font-bold text-purple-700">Franquias</CardTitle>
            </div>
            </CardHeader>
            <CardContent className="pt-0 px-4 pb-4">
              <div className="text-lg font-extrabold text-purple-600 mb-1 break-words">
                {loading ? <CircleNotch size={24} className="animate-spin text-purple-600" /> : 
                  new Set(dadosFiltrados.map(item => item.nm_fantasia)).size
                }
              </div>
              <CardDescription className="text-xs text-gray-500">Franquias com saldo ativo</CardDescription>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-start mb-3">
            <h2 className="text-xl font-bold text-[#000638]">Detalhamento CREDEV por Franquia</h2>
            {dadosFiltrados.length > 0 && (
              <button
              ...
              {loading ? (
                  ...
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-gray-500 font-medium">
                        {dados.length === 0 ? 'Nenhum dado encontrado' : 'Nenhum registro corresponde aos filtros'}
                      </span>
                      <span className="text-gray-400 text-sm">
                        {dados.length === 0 ? 'Não há registros de CREDEV' : 'Tente ajustar os filtros de busca'}
                      </span>
                    </div>
                  </td>
                </tr>
                ...
                sortDados(dadosFiltrados).map((item, index) => (
                  <tr 
                    ...
                    title={`Clique para ver detalhes de auditoria da conta ${item.nr_ctapes || 'N/A'}`}
                  >
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-[10px] text-center text-gray-900">
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <MagnifyingGlass size={10} className="text-blue-500 opacity-70" />
                      </div>
                    </td>
                    <td className="px-2 py-2 whitespace-nowrap text-[10px] text-gray-900">
                      {item.nm_fantasia || '-'}
                    ...
                    <td className="px-2 py-2 whitespace-nowrap">
                      <span className={`inline-flex px-1 py-0.5 text-[9px] font-semibold rounded-full ${
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-[10px] text-right font-semibold">
                      <span className={Number(item.vl_saldo) >= 0 ? 'text-green-600' : 'text-red-600'}>
