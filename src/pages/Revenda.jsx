...
const Revenda = () => {
  ...
  // Frete Revenda (S - E)
  const freteRevenda = React.useMemo(() => {
    return dadosFiltrados.reduce((acc, row) => {
  ...
  // Devoluções Revenda (somatório de entradas E)
  const devolucoesRevenda = React.useMemo(() => {
    return dadosFiltrados.reduce((acc, row) => {
  ...
  React.useEffect(() => {
    ...
    styleElement.textContent = `
      .revenda-table-container {
        overflow-x: auto;
        position: relative;
        max-width: 100%;
      }
      
      .revenda-table-container table {
        position: relative;
      }
      ...
      .revenda-table th:last-child,
      .revenda-table td:last-child {
        border-right: none;
      }
      ...
      .revenda-table tbody tr:nth-child(odd) {
        background-color: white;
      }
      
      .revenda-table tbody tr:nth-child(even) {
        background-color: #fafafa;
      }
      
      .revenda-table tbody tr:hover {
        background-color: #f0f9ff;
        transition: background-color 0.2s ease;
      }
      
      /* CSS para coluna fixa */
      .revenda-table thead th:first-child,
      .revenda-table tbody td:first-child {
      ...
      .revenda-table tbody tr:nth-child(odd) td:first-child {
        background: white !important;
      }
      
      .revenda-table tbody tr:nth-child(even) td:first-child {
        background: #fafafa !important;
      }
      
      .revenda-table tbody tr:hover td:first-child {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col items-stretch justify-start py-8 px-4 pb-8">
        ...
        <div className="rounded-2xl shadow-lg bg-white mt-8 border border-[#000638]/10">
          ...
          {expandTabela && (
            <div className="revenda-table-container overflow-y-auto max-h-[500px]">
              <table className="revenda-table min-w-full text-sm">
                <thead>
                  <tr>
                    <th 
                ...
                <tbody className="overflow-y-auto">
                  {loading ? (
                    <tr><td colSpan={11} className="text-center py-8"><Spinner size={32} className="animate-spin text-blue-600" /></td></tr>
                  ) : dados.length === 0 ? (
                    <tr><td colSpan={11} className="text-center py-8">Nenhum dado encontrado.</td></tr>
                  ) : (
                    sortDadosTransacoes(dadosFiltrados).map((row, i) => {
                      const qtFaturado = Number(row.qt_faturado) || 1;
