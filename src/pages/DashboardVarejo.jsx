...
const DashboardVarejo = () => {
  ...
  const buscarDados = async () => {
    ...
    try {
      ...
      const responseVarejo = await apiClient.sales.faturamentoVarejo({
        dataInicio,
        dataFim,
      });

      // Normalizar diferentes formatos de resposta da API
      const normalizeResponseToArray = (resp) => {
        if (!resp) return [];
        if (Array.isArray(resp)) return resp;
        // Caso padrão: resp.data seja um array
        if (Array.isArray(resp.data)) return resp.data;
        // Algumas respostas vêm como { data: { data: [...] } }
        if (resp.data && Array.isArray(resp.data.data)) return resp.data.data;
        // Às vezes o próprio objeto retornado já é um envelope com propriedades, procurar o primeiro array
        const firstArray = Object.values(resp).find((v) => Array.isArray(v));
        if (firstArray) return firstArray;
        return [];
      };

      const normalizedVarejo = normalizeResponseToArray(responseVarejo);
      setDadosVarejo(normalizedVarejo);
      ...
      const responseMesAnterior = await apiClient.sales.faturamentoVarejo({
      ...
      setError('Erro ao carregar dados de faturamento do varejo');
    } finally {
      setLoading(false);
    }
  };

  // Consolidar dados do varejo
  const dadosConsolidados = useMemo(() => {
    if (!dadosVarejo.length) return null;

    const consolidado = dadosVarejo.reduce(
      (acc, item) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      <PageTitle
        title="Dashboard Varejo"
        subtitle="Análise completa do faturamento do canal Varejo"
        icon={Storefront}
      ...
      {loading ? (
        ...
        <>
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl rounded-xl bg-white mb-6">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                <Storefront size={20} className="text-[#000638]" />
                <CardTitle className="text-lg font-bold text-[#000638]">
                  Detalhamento Varejo
                </CardTitle>
              </div>
              <CardDescription>
                Resumo financeiro completo do canal Varejo
              </CardDescription>
            </CardHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            {dadosPizzaEmpresa && (
              <Card
                ...
                onClick={() =>
                  setModalGrafico({
                    title: 'Varejo - Faturamento por Empresa',
...
export default DashboardVarejo;
