...
const DashboardFranquias = () => {
  ...
  const [dadosFranquias, setDadosFranquias] = useState([]);
  ...
  const buscarDados = async () => {
    ...
    try {
      ...
      const responseFranquias = await apiClient.sales.faturamentoFranquias({
        dataInicio,
        dataFim,
      });
      ...
      const normalizedFranquias = normalizeResponseToArray(responseFranquias);
      setDadosFranquias(normalizedFranquias);
      ...
      const responseMesAnterior = await apiClient.sales.faturamentoFranquias({
      ...
      setError('Erro ao carregar dados de faturamento de franquias');
    } finally {
      setLoading(false);
    }
  };

  // Consolidar dados das franquias
  const dadosConsolidados = useMemo(() => {
    if (!dadosFranquias.length) return null;

    console.log('📊 Consolidando dados de Franquias...');

    const consolidado = dadosFranquias.reduce(
      (acc, item) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4">
      <PageTitle
        title="Dashboard Franquias"
        subtitle="Análise completa do faturamento do canal Franquias"
        icon={Handshake}
      ...
      {loading ? (
        ...
        <>
          ...
          <Card className="shadow-lg transition-all duration-200 hover:shadow-xl rounded-xl bg-white mb-6">
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                <Handshake size={20} className="text-[#000638]" />
                <CardTitle className="text-lg font-bold text-[#000638]">
                  Detalhamento Franquias
                </CardTitle>
              </div>
              <CardDescription>
                Resumo financeiro completo do canal Franquias
              </CardDescription>
            </CardHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            {dadosPizzaEmpresa && (
              <Card
                ...
                onClick={() =>
                  setModalGrafico({
                    title: 'Franquias - Faturamento por Empresa',
              ...
              >
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-lg font-bold text-[#000638]">
                      Franquias - Faturamento por Empresa
                    </CardTitle>
                  </div>
                  <CardDescription>
                    Distribuição do faturamento por empresa (Franquias)
                  </CardDescription>
                </CardHeader>
                <CardContent>
...
export default DashboardFranquias;
