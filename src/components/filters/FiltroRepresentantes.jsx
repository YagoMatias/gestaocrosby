...
const FiltroRepresentantes = ({
  ...
  const toggle = (nome) => {
    if (selected.includes(nome)) onChange(selected.filter((s) => s !== nome));
    else onChange([...selected, nome]);
  };

  const selecionarTodos = () => onChange(representantes.slice());
  const limpar = () => onChange([]);

  const resumo = useMemo(() => {
    if (!selected || selected.length === 0) return 'Todos os representantes';
              ...
              .map((rep) => (
                ...
                >
                  ...
                  <div className="flex flex-row w-full ml-2">
                    <span className="text-xs text-[#000638]">{rep}</span>
                  </div>
                </label>
              ))}
...
export default FiltroRepresentantes;
