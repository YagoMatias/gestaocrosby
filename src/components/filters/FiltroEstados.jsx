...
const FiltroEstados = ({
  ...
  const toggle = (sigla) => {
    if (selected.includes(sigla)) onChange(selected.filter((s) => s !== sigla));
    else onChange([...selected, sigla]);
  };

  const selecionarTodos = () => onChange(estados.slice());
  const limpar = () => onChange([]);

  const resumo = useMemo(() => {
    if (!selected || selected.length === 0) return 'Todos os estados';
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {showDropdown && (
        <div className="w-1/2 absolute top-full left-0 right-0 z-50 bg-white border border-gray-300 rounded-lg shadow-lg mt-1 max-h-100 overflow-hidden">
          ...
          <div className="w-full max-h-48 overflow-y-auto">
            {estados
              .filter((est) =>
                est.toLowerCase().includes(searchTerm.toLowerCase()),
              )
              .map((est) => (
                <label
                  key={est}
                ...
                >
                  <input
                    type="checkbox"
                    checked={selected.includes(est)}
                    onChange={() => toggle(est)}
                  />
                  <div className="flex flex-row w-full ml-2">
                    <span className="text-xs text-[#000638]">{est}</span>
                  </div>
                </label>
              ))}
            {estados.filter((est) =>
