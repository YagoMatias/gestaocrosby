...
import { useTabContext } from './FinanceiroTabs/TabContext';
import {
  UserGear,
  X,
  Money,
...
const Sidebar = ({ isOpen, onClose, onToggle }) => {
  ...
  const handleNavigation = useCallback(
    (href, external = false) => {
      ...
      } else if (tabCtx && tabCtx.isFinanceiroPath(href)) {
        tabCtx.openTab(href);
        navigate(href);
      } else {
        navigate(href);
    ...
    <div className="w-60 h-full bg-white shadow-xl border-r border-gray-200 flex flex-col">
      ...
      <nav className="flex-1 px-2 py-4 space-y-3 overflow-y-auto overflow-x-hidden">
        ...
        <MenuSection
          ...
          items={solicitacoesCrosby}
          isOpen={openSection === 'solicitacoesCrosby'}
