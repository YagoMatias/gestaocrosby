...
const FiltroCliente = ({ clientesSelecionados = [], onSelectClientes, dadosClientes = [] }) => {
  ...
  const clientesFiltrados = dadosClientes.filter(cliente =>
    cliente.cd_cliente?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
    cliente.nm_cliente?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleToggleCliente = (clienteObj) => {
    let novosSelecionados;
    if (clientesSelecionados.some(cliente => cliente.cd_cliente?.toString() === clienteObj.cd_cliente?.toString())) {
      novosSelecionados = clientesSelecionados.filter(cliente => cliente.cd_cliente?.toString() !== clienteObj.cd_cliente?.toString());
    } else {
      novosSelecionados = [...clientesSelecionados, clienteObj];
    }
    if (onSelectClientes) onSelectClientes(novosSelecionados);
  };
        ...
        <div className="w-80 absolute top-full left-0 right-0 z-50 bg-white border border-gray-300 rounded-lg shadow-lg mt-1 max-h-100 overflow-hidden">
          ...
          <div className="p-3 border-b border-gray-200 bg-gray-50 flex gap-2">
            <button
              type="button"
              onClick={() => {
                if (onSelectClientes) onSelectClientes([...dadosClientes]);
              }}
            ...
            <button
              type="button"
              onClick={() => {
                if (onSelectClientes) onSelectClientes([]);
              }}
          ...
          <div className="w-full max-h-48 overflow-y-auto">
            {dadosClientes.length === 0 ? (
              ...
              clientesFiltrados.map((cliente) => {
                const isSelected = clientesSelecionados.some(cli => cli.cd_cliente?.toString() === cliente.cd_cliente?.toString());
                return (
                  <div
