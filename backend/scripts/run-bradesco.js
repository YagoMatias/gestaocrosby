import { processBradescoExtracts } from '../utils/extratos/bradescoExtractor.js';
...
function printTransactions(transactions, max = 50) {
  ...
  transactions.slice(0, max).forEach((t, i) => {
    console.log(
      `${i + 1}. ${t.data} | ${t.descricao} | valor=${t.valor} | saldo=${t.saldo}`
    );
  });
}

async function main() {
  const results = await processBradescoExtracts();
  console.log('Resumo dos PDFs BRADESCO processados:');
...
main().catch((err) => {
  console.error('Falha geral ao processar BRADESCO:', err);
  process.exit(1);
});
