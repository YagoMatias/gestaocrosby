...
const UNICRED_DIR = path.resolve(__dirname, '../../EXTRATO NOVEMBRO/UNICRED');
...
function extractTransactionsFromText(text) {
  // Unicred costuma ter tabela com Data | Histórico | Documento | Valor | Saldo
  const dateRegex = /(\b\d{2}\/\d{2}(?:\/\d{4})?\b)/g;
  const moneyRegex = /R\$\s?-?[0-9\.]+,[0-9]{2}|-?[0-9\.]+,[0-9]{2}/g;
  const skipPatterns = [/^extrato/i, /^agência/i, /^conta/i, /^saldo\s+anterior/i, /^total/i];
  ...
  while ((m = dateRegex.exec(text)) !== null) {
    idx.push({ index: m.index, token: m[0] });
  }
  for (let i = 0; i < idx.length; i++) {
    const start = idx[i].index;
...
export async function processUnicredExtracts() {
  const files = listPdfFiles(UNICRED_DIR);
  const results = [];
