...
const BB_DIR = path.resolve(__dirname, '..', '..', 'EXTRATO NOVEMBRO', 'BB');
...
// Heurística para Banco do Brasil:
// Muitos extratos BB têm colunas "Data | Histórico | Documento | Valor | Saldo".
// Valorizamos valores com ou sem prefixo R$, e com possível sinal negativo.
...
export async function processBBExtracts() {
  const pdfFiles = listPdfFiles(BB_DIR);
  const results = [];
